...
const tableSchemas = [
  ...
  // 2. User Roles
  {
    ...
    attributes: [
      { key: "user_id", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["reader", "writer", "editor", "moderator", "admin"],
      },
  ...
  // 3. Novels
  {
    ...
    attributes: [
      ...
      { key: "author_id", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["draft", "ongoing", "completed", "hiatus", "cancelled"],
        default: "draft",
      },
      {
        key: "visibility",
        type: "enum",
        required: true,
        elements: ["public", "private", "unlisted", "password_protected"],
        default: "public",
      },
    ...
    indexes: [
      {
        key: "novel_id_volume",
        type: "unique",
        attributes: ["novel_id", "volume_number"],
        orders: ["ASC", "ASC"],
      },
      {
        key: "novel_id",
  ...
  // 5. Chapters
  {
    ...
    attributes: [
      ...
      { key: "notes", type: "string", required: false, size: 1000 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["draft", "scheduled", "published", "archived"],
        default: "draft",
      },
      {
        key: "visibility",
        type: "enum",
        required: true,
        elements: ["public", "private", "password_protected"],
        default: "public",
      },
      { key: "password", type: "string", required: false, size: 255 },
      { key: "is_free", type: "boolean", required: false, default: false },
  ...
  // 8. Reading History
  {
    ...
    indexes: [
      {
        key: "user_id_novel_chapter",
        type: "unique",
        attributes: ["user_id", "novel_id", "chapter_id"],
        orders: ["ASC", "ASC", "ASC"],
      },
      { key: "user_id", type: "key", attributes: ["user_id"], orders: ["ASC"] },
      {
  ...
  // 15. Reports
  {
    ...
    attributes: [
      { key: "reporter_id", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["novel", "chapter", "comment", "user"],
      },
      ...
      { key: "description", type: "string", required: false, size: 1000 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["pending", "reviewed", "resolved", "dismissed"],
        default: "pending",
      },
  ...
  // 23. Collection Novels
  {
    ...
    indexes: [
      {
        key: "collection_novel",
        type: "unique",
        attributes: ["collection_id", "novel_id"],
        orders: ["ASC", "ASC"],
      },
      {
        key: "collection_id",
    ...
    indexes: [
      {
        key: "writer_follower",
        type: "unique",
        attributes: ["writer_id", "follower_id"],
        orders: ["ASC", "ASC"],
      },
      {
        key: "writer_id",
  ...
  // 29. Coin Transactions
  {
    ...
    attributes: [
      { key: "user_id", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["earned", "spent", "received", "purchased", "bonus"],
      },
  ...
  // 30. Purchases
  {
    ...
    attributes: [
      { key: "user_id", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["coins", "subscription", "chapter"],
      },
  ...
  // 31. Subscriptions
  {
    ...
    attributes: [
      { key: "user_id", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["basic", "premium", "pro"],
      },
      {
        key: "status",
        type: "enum",
        required: true,
        elements: ["inactive", "active", "cancelled", "expired"],
        default: "inactive",
      },
  ...
  // 33. Premium Chapters
  {
    ...
    attributes: [
      ...
      { key: "coins_spent", type: "integer", required: false, default: 0 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["coins", "wait", "subscription", "promo"],
        default: "coins",
      },
      { key: "metadata", type: "json", required: false, default: "{}" },
      { key: "is_deleted", type: "boolean", required: false, default: false },
  ...
  // 34. Coupons
  {
    ...
    attributes: [
      { key: "code", type: "string", required: true, size: 255 },
      {
        ...
        type: "enum",
        required: true,
        elements: ["fixed_amount", "percentage", "free_chapters", "free_coins"],
      },
...
// Helper function to get existing attributes of a collection
async function getCollectionAttributes(
  ...
  try {
    ...
    console.error(`Error getting attributes for ${collectionId}:`, err.message);
    return [];
  }
}

// Helper function to add missing attributes to existing collection
async function addMissingAttributes(collectionId: string, attributes: any[]) {
  const existingAttrs = await getCollectionAttributes(collectionId);
...
async function setupAppwrite() {
  ...
  for (const schema of tableSchemas) {
    console.log(`\nProcessing table: ${schema.name} (${schema.id})`);

    const exists = await collectionExists(schema.id);

    if (!exists) {
      console.log(`Creating new table: ${schema.id}`);
      try {
        // Create collection
      ...
      console.log(`Table ${schema.id} already exists, updating schema...`);

      // Add missing attributes to existing collection
      await addMissingAttributes(schema.id, schema.attributes);

      console.log(`✅ Table ${schema.id} updated`);
    }
  }
